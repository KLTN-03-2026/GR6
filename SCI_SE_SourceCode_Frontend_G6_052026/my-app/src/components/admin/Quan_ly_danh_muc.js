...
const Quan_ly_danh_muc = () => {
  ...
  // States cho Form (Chỉ tập trung 3 trường: name, id_father, hinh_anh)
  ...
  const handleFileChange = (e) => {
    ...
    if (file) {
      // Kiểm tra dung lượng file (Laravel thường giới hạn 2MB)
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      // Chỉ append nếu là file mới chọn, nếu là string (URL cũ) thì bỏ qua
      ...
      if (editingId) {
        ...
        // Với FormData và Laravel, dùng POST kèm _method=PUT thường ổn định hơn PUT trực tiếp
      ...
      setError(err.response?.data?.message || 'Lỗi hệ thống khi lưu dữ liệu');
    } finally {
      ...
      <main className="flex-grow p-10">
        <div className="max-w-7xl mx-auto">
          ...
          <div className="flex gap-3 mb-8 overflow-x-auto pb-4 no-scrollbar">
            <button onClick={() => { setSelectedFatherId(0); handleReset(); }} className={`px-6 py-3 rounded-xl font-bold text-sm transition-all border whitespace-nowrap ${selectedFatherId === 0 ? 'bg-blue-600 text-white border-blue-600 shadow-md' : 'bg-white text-gray-500 border-gray-100 hover:border-blue-200'}`}>Tất cả gốc</button>
                  ...
                  <div className="relative">
                    ...
                    >
                      <span className={formData.id_father === 0 ? "text-blue-600 font-bold" : "text-slate-600 font-medium"}>
                        {formData.id_father === 0 ? "Danh mục gốc" : currentFather?.ten_dich_vu}
                      </span>
